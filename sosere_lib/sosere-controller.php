 ...
 */
 ...
 if ( ! class_exists( 'Sosere_Controller' ) ) {
	class Sosere_Controller
	{
	 ...
	 private $recommedation_box_title = 'Read more';
	 ...
	 */
		function __construct() {
			if( ! is_admin() ) {
				...
				if ( isset( $this->array_sosere_options['recommedation_box_title'] ) ) $this->recommedation_box_title = $this->array_sosere_options['recommedation_box_title']; 
		...
		*/		 
		public function sosere_run( $content ) {
			...
			if ( is_single() || is_page() ) {
				...
				// get post content
				$selected_posts_arr = get_posts( array(
						'include'          => implode( ',', $selected_post_IDs ),
				...
				if ( true == $this->use_cache ) {
					if ( isset( $selected_posts_arr ) ) {
								add_post_meta( $this->post->ID, 'soseredbviewedpostscache', $recommendation_string, true ) or
								update_post_meta( $this->post->ID, 'soseredbviewedpostscache', $recommendation_string );
								add_post_meta( $this->post->ID, 'soseredbviewedpostscachedate', $this->now, true ) or
								update_post_meta( $this->post->ID, 'soseredbviewedpostscachedate', $this->now );
							}
				}
		 ...
		 */
		 private function get_html_output( $selected_posts ) {
			...
			$return_string .= '</ul></div><!-- end sosere recommendation output -->';
